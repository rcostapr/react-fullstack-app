...
Builds the app for production to the `build` folder.<br> It correctly bundles React in
...
- use reactdb
- Create User to database reactdb

```
db.createUser({
    user: "reactuser",
    pwd: "react",
    roles: [ { role: "dbAdmin", db: "reactdb" } ]
})
```
